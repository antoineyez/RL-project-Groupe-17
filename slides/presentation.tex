...
\begin{frame}{Protocole exp\'erimental}
    ...
    \begin{table}
        ...
        \begin{tabular}{lcc}
            ...
            R\'eseau & 256-128 (custom) & \texttt{MlpPolicy} (SB3) \\
            \bottomrule
        \end{tabular}
        }
    \end{table}
...
\begin{frame}{Courbes d'entra\^inement}
    ...
    \vspace{0.2cm}
    {\small Moyenne $\pm$ \'ecart-type sur 3 seeds. Lissage par moyenne glissante (fenetre 10).}

    \vspace{0.2cm}
    \begin{itemize}
        \item Notre DQN converge en $\sim$5\,000 timesteps. SB3 en $\sim$20\,000.
        \item Variance inter-seeds beaucoup plus faible pour notre DQN.
    \end{itemize}
\end{frame}
...
\begin{frame}{\'Evaluation quantitative (50 runs $\times$ 3 seeds)}
    \begin{table}
        ...
        \begin{tabular}{lcccc}
            ...
            \midrule
            DQN (custom) & 42 & 20.68 & 2.70 & 6/50 (12\%) \\
            DQN (custom) & 43 & 20.33 & 2.60 & 2/50 (4\%) \\
            DQN (custom) & 44 & 20.53 & 2.29 & 4/50 (8\%) \\
            \textbf{DQN (custom)} & \textbf{total} & \textbf{20.51} & \textbf{2.54} & \textbf{12/150 (8\%)} \\
            \midrule
            SB3 DQN & 42 & 19.52 & 3.47 & 13/50 (26\%) \\
            SB3 DQN & 43 & 16.11 & 6.00 & 22/50 (44\%) \\
            SB3 DQN & 44 & 10.77 & 7.35 & 42/50 (84\%) \\
            \textbf{SB3 DQN} & \textbf{total} & \textbf{15.47} & \textbf{6.85} & \textbf{77/150 (51\%)} \\
            \bottomrule
        \end{tabular}
    \end{table}

    \vspace{0.2cm}
    Notre DQN : reward sup\'erieure (+33\%), variance plus faible, et \textbf{6$\times$ moins de crashes}.
\end{frame}
...
\begin{frame}{Analyse des modes d'\'echec}
    \begin{columns}
        \begin{column}{0.5\textwidth}
            \textbf{Notre DQN (8\% de crashes)}

            \vspace{0.2cm}
            Deux patterns identifi\'es :
            \begin{itemize}
                \item \textbf{Crash pr\'ecoce} (steps 3--9) : changement de voie dans une voie occup\'ee juste apr\`es le reset.
                \item \textbf{Crash tardif} (steps 21--29) : collision en fin d'\'episode lors d'un changement de voie dans du trafic rapide.
            \end{itemize}
        \end{column}
        \begin{column}{0.5\textwidth}
            \textbf{SB3 DQN (51\% de crashes)}

            \vspace{0.2cm}
            Modes d'\'echec plus vari\'es :
            \begin{itemize}
                \item Crashes \`a haute vitesse (jusqu'\`a 28 m/s)
                \item Forte d\'ependance au seed : seed 44 $\Rightarrow$ \textbf{84\% de crashes}
                \item Politique fragile : fonctionne sur certaines configs de trafic, pas d'autres
            \end{itemize}
        \end{column}
    \end{columns}
    ...
    \begin{block}{Hypoth\`ese}
        SB3 souffre de son \texttt{learning\_starts=1000} (pas d'apprentissage pendant les 1000 premiers pas) et d'un schedule d'exploration plus court (30\% vs 50\%).
    \end{block}
\end{frame}
...
\begin{frame}{Choix de design}
    \begin{itemize}
        ...
        \vspace{0.2cm}
        \item \textbf{MLP 256-128 :} suffisant pour un espace d'\'etat de dimension 50. Un r\'eseau plus grand risquerait l'overfitting sans apporter de capacit\'e de repr\'esentation utile.
...
\begin{frame}{Conclusion}
    ...
    \begin{itemize}
        \item Notre DQN custom \textbf{surpasse SB3} : reward moyenne 20.51 vs 15.47, taux de crash 8\% vs 51\%.
        \item Convergence plus rapide ($\sim$5k vs $\sim$20k timesteps) et plus reproductible (faible variance inter-seeds).
    \end{itemize}

    \vspace{0.3cm}
    \textbf{Le\c{c}ons apprises :}
    \begin{itemize}
        \item L'importance d'une comparaison \'equitable (m\^eme unit\'e, m\^eme config).
    ...
    \begin{itemize}
        ...
        \item Tuning d'hyperparamètres (exploration schedule, architecture)
